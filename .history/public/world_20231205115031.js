...
function placeObjectSafely(createFunc, scene, objectType, maxAttempts = 100) {
    let success = false;
    let position;
    ...
    for (let attempts = 0; attempts < maxAttempts; attempts++) {
        ...
        let overlapSize = objectType === 'tree' ? 10 : 20; // Different sizes for different objects
    ...
    if (!success) {
        console.log(`Unable to place ${objectType} after ${maxAttempts} attempts. Adjusting position.`);
        position = adjustPositionForCreation(scene, objectType);
        createFunc(scene, position.x, position.z, objectType);
    }
}

function adjustPositionForCreation(scene, objectType, searchRadius = 100, step = 5) {
    let position;
    let found = false;

    for (let radius = 1; radius <= searchRadius; radius += step) {
        for (let angle = 0; angle < 360; angle += 10) { // Adjust the angle step for finer search
            let x = radius * Math.cos(angle * Math.PI / 180);
            let z = radius * Math.sin(angle * Math.PI / 180);
            position = new BABYLON.Vector3(x, 0, z);

            if (!isOverlapping(position, objectPositions, objectType === 'tree' ? 10 : 20)) {
                found = true;
                break;
            }
        }
    ...
    if (!found) {
        ...
        return new BABYLON.Vector3(0, 0, 0); // Fallback to a default 
...
function adjustPosition(scene, objectType) {
    ...
    } while (isOverlapping(new BABYLON.Vector3(x, 0, z), objectPositions, objectType === 'tree' ? 10 : 20));
