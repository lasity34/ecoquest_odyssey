...
function placeObjectSafely(createFunc, scene, objectType, maxAttempts = 200) {
  ...
  // Check if the object of this type already exists
  let existingObject = objectPositions.find(p => p.type === objectType);
  if (existingObject) {
    // Object already exists, use the existing position
    createFunc(scene, existingObject.position.x, existingObject.position.z, objectType);
    console.log(`Using existing position for ${objectType}`);
    return; // Exit the function as the object is already placed
  }

  // If object does not exist, find a new position
  let success = false;
  let position;
  ...
  for (let attempts = 0; attempts < maxAttempts; attempts++) {
    ...
    if (!isOverlapping(position, objectPositions, overlapSize)) {
      ...
      objectPositions.push({ position: position, type: objectType }); // Save new position
      savePositions(); // Persist new positions
      success = true;
      console.log(`Placed new ${objectType}`);
      break;
    }
  }
