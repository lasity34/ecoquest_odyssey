...
function placeObjectSafely(createFunc, scene, objectType) {
    let maxAttempts = 100; // Increased maximum attempts
    let success = false;
    ...
    if (!success) {
        console.log(`${objectType} could not be placed without overlap. Placing at default location.`);
        createFunc(scene, 0, 0, objectType); // Default location or some other strategy
    }
}
