...
function placeObjectSafely(createFunc, scene, objectType) {
    ...
    for (let attempts = 0; attempts < maxAttempts; attempts++) {
        let x = Math.random() * 150 - 50; // Adjust the range based on your scene
        let z = Math.random() * 150 - 50;
        let overlapSize = objectType === 'tree' ? 10 : 20; // Different sizes for different objects

        if (!isOverlapping(new BABYLON.Vector3(x, 0, z), objectPositions, overlapSize)) {
    ...
    if (!success) {
        console.log(`${objectType} placement adjusted due to overlap.`);
        // Adjust the position slightly until a non-overlapping position is found
        let adjustedPosition = adjustPosition(scene, objectType);
        createFunc(scene, adjustedPosition.x, adjustedPosition.z, objectType);
    }
}
